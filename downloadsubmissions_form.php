 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_downloadsubmissions_settings_form extends mod_quiz_attempts_report_form {

    protected function other_preference_fields(MoodleQuickForm $mform) {
        $mform->addGroup(array(
            $mform->createElement('advcheckbox', 'qtext', '',
                get_string('questiontext', 'quiz_downloadsubmissions')),
            $mform->createElement('advcheckbox', 'resp', '',
                get_string('response', 'quiz_downloadsubmissions')),
            $mform->createElement('advcheckbox', 'right', '',
                get_string('rightanswer', 'quiz_downloadsubmissions')),
        ), 'coloptions', get_string('showthe', 'quiz_downloadsubmissions'), array(' '), false);
