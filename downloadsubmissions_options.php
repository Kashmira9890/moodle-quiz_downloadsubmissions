...
/**
 * Class to store the options for a {@link quiz_downloadsubmissions_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_downloadsubmissions_options extends mod_quiz_attempts_report_options {
    ...
    public function setup_from_user_preferences() {
        ...
        $this->showqtext     = get_user_preferences('quiz_report_downloadsubmissions_qtext', $this->showqtext);
        $this->showresponses = get_user_preferences('quiz_report_downloadsubmissions_resp',  $this->showresponses);
        $this->showright     = get_user_preferences('quiz_report_downloadsubmissions_right', $this->showright);
        if (quiz_allows_multiple_tries($this->quiz)) {
            $this->whichtries    = get_user_preferences('quiz_report_downloadsubmissions_which_tries', $this->whichtries);
        }
    }
    ...
    public function update_user_preferences() {
        ...
        set_user_preference('quiz_report_downloadsubmissions_qtext', $this->showqtext);
        set_user_preference('quiz_report_downloadsubmissions_resp',  $this->showresponses);
        set_user_preference('quiz_report_downloadsubmissions_right', $this->showright);
        if (quiz_allows_multiple_tries($this->quiz)) {
            set_user_preference('quiz_report_downloadsubmissions_which_tries', $this->whichtries);
        }
    }
